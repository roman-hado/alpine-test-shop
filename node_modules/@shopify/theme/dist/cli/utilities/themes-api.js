...
async function request(method, path, session, params) {
...
function buildTheme(themeJson) {
    ...
    return new Theme(themeJson.id, themeJson.name, themeJson.created_at, themeJson.updated_at, themeJson.role, themeJson.theme_store_id, themeJson.previewable, themeJson.processing, themeJson.admin_graphql_api_id);
}
function handleForbiddenError(session) {
    const store = session.storeFqdn;
    const adminUrl = storeAdminUrl(session);
    throw new error.Abort(`You are not authorized to edit themes on "${store}".`, "You can't use Shopify CLI with development stores if you only have Partner staff " +
